...
def run(filename):
    ...
    for command in commands:
        if (command['op'] == "push"):
            ...
            q = make_translate(command['args'][0],command['args'][1],command['args'][2])
            matrix_mult(stack[-1], q)
            stack[-1] = [w[:] for w in q]
        elif(command['op'] == "scale"):
            q = make_scale(command['args'][0],command['args'][1],command['args'][2])
            matrix_mult(stack[-1],q)
            stack[-1] = [w[:] for w in q]
        elif(command['op'] == "rotate"):
            q = new_matrix()
            if(command['args'][0] == 'x'):
                q = make_rotX(command['args'][1]*fred)
            elif(command['args'][0] == 'y'):
                q = make_rotY(command['args'][1]*fred)
            elif(command['args'][0] == 'z'):
                q = make_rotZ(command['args'][1]*fred)
            else:
            ...
            print(command['op'],", invalid input")
        # print command
